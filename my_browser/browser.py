...
from PyQt5.QtWidgets import (
    QApplication, QMainWindow, QToolBar, QAction, QLineEdit, QTabWidget, QToolButton, QWidget,
    QMenu, QFileDialog, QMessageBox, QTabBar, QLabel
)
...
class MyBrowser(QMainWindow):
    ...
    def create_navbar(self):
        ...
        self.url_bar.setPlaceholderText("Arama...")
        self.url_bar.returnPressed.connect(self.load_url_or_search)
        navbar.addWidget(self.url_bar)

        self.bookmark_menu= QMenu("Yer İmleri",self)
        navbar.addAction(self.bookmark_menu.menuAction())

        add_bookmark_action=QAction("Yer İmlerine Ekle", self)
        add_bookmark_action.triggered.connect(self.add_bookmark)
        navbar.addAction(add_bookmark_action)

        self.history_menu= QMenu("Geçmiş", self)
        navbar.addAction(self.history_menu.menuAction())
    ...
    def load_url_or_search(self):
        try:
            ...
            print(f"Bir hata oluştu (Url Yüklerken):{e} ")
    ...
    def update_url_bar(self):
        try:
            ...
            print(f"Bir hata oluştu (Url çubuğunu güncellerken): {e}")
    ...
    def update_url(self, q, browser=None):
        try:
            ...
            print(f"Bir hata oluştu (URL Güncellerken): {e}")
    ...
    def update_tab_title(self,title,browser):
        try:
            ...
            print(f"Bir hata oluştu. (Sekme başlığı güncellenirken): {e}")
    ...
    def add_to_history(self):
        ...
        if isinstance(current_widget, QWebEngineView):
            ...
            if url not in [entry['url'] for entry in self.history]:
                ...
                if len(url)> 50:
                    url_summary= url[:50]+"..."
                history_action = QAction(f"{visit_time} - {url_summary}",self)
